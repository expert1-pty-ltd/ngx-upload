...
describe('HttpClientUploadService', () => {
  ...
  it('should add 3 files in the queue', () => {
    const files = new FileAPI.FileList(
      new FileAPI.File('./image.jpg'),
      new FileAPI.File('./image2.jpg'),
      new FileAPI.File('./image3.jpg')
    );
    expect(httpClientUploadService.queue.length).toBe(0);
    const dropOptions: DropTargetOptions = {
      color: '',
      colorDrag: '',
      colorDrop: '',
      multiple: true,
      accept: [MimeTypeEnum.Image_Jpeg],
      disableMultipart: false,
    };
  ...
  it('should add 3 files with different types in the queue', () => {
    ...
    const dropOptions: DropTargetOptions = {
      ...
      colorDrop: '',
      multiple: true,
      accept: [MimeTypeEnum.Image_Jpeg, MimeTypeEnum.Application_Pdf],
      disableMultipart: false,
    };
    httpClientUploadService.addToQueue(files, null, dropOptions);
    expect(httpClientUploadService.queue.length).toBe(3);
  ...
  it('should set FileItem with a true disable multipart when we use InputFileOptions', () => {
    const files = new FileAPI.FileList(new FileAPI.File('./image.jpg'));
    expect(httpClientUploadService.queue.length).toBe(0);
    const dropOptions: InputFileOptions = {
      multiple: true,
      accept: [MimeTypeEnum.Image_Jpeg],
      disableMultipart: true,
    };
  ...
  it('should remove all fileItem ', () => {
    ...
    expect(httpClientUploadService.queue.length).toBe(3);
    expect(
      httpClientUploadService.queue.filter((item) => item.isReady).length
    ).toBe(3);
  });
  ...
  it('should upload fileItem ', () => {
    ...
    httpClientUploadService.addToQueue(files, null, this.ngxDropTargetOptions);
    httpClientUploadService.uploadFileItem(
      httpClientUploadService.queue[0],
      endpoint
    );
